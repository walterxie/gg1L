...
ggPercentageBarChart <- function(df.to.melt, melt.id, title="Percentage Bar Chart",
  ...
  if (! is.null(palette)) {
    ...
    pale <- gg1L::getMyPalette(length(legend.ord))
  ...
  p <- gg1L::ggBarChart(df.melt, x.id="variable", y.id="value", fill.id=melt.id,
...
ggGroupAbundanceBar <- function(df.to.melt, melt.id, colour.id=NULL, prop.thre=0,
  ...
  p <- gg1L::ggBarChart(df.melt, x.id=melt.id[1], y.id="value", fill.id=colour.id,
